...
colors = ['r', 'g']
linestyles = "-","--"
for j, source in enumerate(["transpose","transpose-blocked"]):
    ...
    for i, data in enumerate([data_tens, data_non_tens]):
        ax.plot(data["Nrow"], data["EffectiveBW"], marker='o', linestyle=linestyles[j], color=colors[i], label=f"{source} {'tens' if i==0 else '2^n'}")
ax.legend()
plt.show()
